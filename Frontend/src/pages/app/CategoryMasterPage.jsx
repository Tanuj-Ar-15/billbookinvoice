...
const CategoryMasterPage = () => {
  ...
  const { loading, error, list, pagination } = useSelector(
  ...
  const loadPage = (page) => {
    dispatch(fetchCategories({ page, search: appliedSearch }))
  }

  const applySearch = () => {
    const q = localSearch.trim()
    setAppliedSearch(q)
    dispatch(fetchCategories({ page: 1, search: q }))
  }
  ...
  const handleCreate = async (e) => {
    ...
    setName('')
    dispatch(fetchCategories({ page: 1, search: appliedSearch }))
  }
  ...
  const saveEdit = async () => {
    ...
    await dispatch(updateCategory({ id: editId, name: editName.trim() }))
    setEditId(null)
    dispatch(fetchCategories({ page: pagination.page, search: appliedSearch }))
  }
  ...
  const clearErr = () => dispatch(clearCategoryError())
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
          Create categories for your menu items.
        </p>
      </div>
